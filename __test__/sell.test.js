...
describe("post /sell",()=> {
    ...
    describe("testing their wallet is decrease", ()=> {
        ...
        //test success after selling
        
        it("will success when sell", async ()=>{
            await db.connect(async (db) => {
                ...
                var result = await agent.post("/api/sell")
                .send({
        ...
        it("will check the wallet increases", async ()=>{
            var flag = false;
            if(newWallet > oldWallet) flag = true;
            expect(flag).toBe(true);
        })

        it("will check the numStock decrease", async ()=> {
            var flag = false;
