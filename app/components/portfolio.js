...
export default function Portfolio({symbol, price}) {
    ...
    return (
    <div className="max-w-lg m-auto py-2">
        {stocks.map((stock) => {
            return (
            <div className ="pb-1">
                <Link href={`/market/${stock.symbol}`}>
                    <div className="drop-shadow-md py-2 flex max-w-3xl items-center m-1 bg-zinc-50 justify-between hover:bg-slate-200 dark:hover:bg-slate-400 rounded-md">
                        <div className = "flex items-center justify-self-end">
                            ...
                            <p className = "font-bold text-lg mx-1 justify-self-end">{stock.symbol}</p>
                        </div>
                        
                        <p className = "font-bold text-lg px-1">${stock.value.toFixed(2)}</p>
                        {stock.percentage > 0 &&
