...
export default function Overview({symbol, quote}) {
    ...
    return (
        ...
        <div className="flex items-center mb-1">
          ...
          <h1 className="font-bold mx-2 text-xl uppercase interBold">{symbol.toUpperCase()}:</h1>
          <p className = "interBold">${quote}</p>
        </div>
        <div className="flex items-center justify-center">
            <img src={`https://logos.stockanalysis.com/${symbol.toLowerCase()}.svg`} width="50" height="50"/>
            <h1 className="font-bold text-xl text-center category">{companyName}</h1>
        </div>
        <h3 className="text-lg text-center interBold">{category}</h3>
        </>);
} 