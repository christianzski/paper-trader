    ...
    api: async function(req, res) {
        try {
            ...
            if (user) {
                ...
                await db.connect(async (db) => {
                    const portfolio = await db.collection('Portfolio').findOne({userId: id});

                    res.setHeader('Content-Type', 'application/json');

                    if(portfolio) {
                        if(range == "1d") {
                            res.send(JSON.stringify( {data: portfolio.portHistoryValueDay} ));
                        } else if(range == "1w") {
                            let month = portfolio.portHistoryValueMonth;
                            res.send(JSON.stringify( {data: month.slice(month.length - 1 - 7, month.length - 1) }));
                        } else if(range == "1m") {
                            res.send(JSON.stringify( {data: portfolio.portHistoryValueMonth} ));
                        } else {
                            res.send(JSON.stringify( {error: "invalid range"} ));
                        }
                    } else {
